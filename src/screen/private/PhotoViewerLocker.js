...
export default function PhotoViewerLocker({navigation, route}) {
  const {data, indx} = route.params;
  const [delImage, setDelImage] = useState();
  const [delAlert, setDelAlert] = useState(false);
  const [comeData, setData] = useState();
  useEffect(() => {
    const backAction = () => {
      navigation.goBack();
      return true;
  ...
  async function deletehandler(index, filepath) {
    ...
    if (exists) {
      ...
      comeData.splice(index, 1);
      storeData(comeData);
      setDelAlert(false);
      navigation.navigation(All_Strings.HIDE_GALLERY, {
        dataComIn: data,
        deleteImg: true,
      });
    } else {
                  ...
                  <View
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        setDelAlert(false);
                        console.log('Press successfully............!', comeData);
                        // navigation.navigate(All_Strings.HIDE_GALLERY,{i:index,deleteImg:true})
